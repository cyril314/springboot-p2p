						...
						<table cellspacing="1" cellpadding="3">
							...
							<tr>
								<td class="blue12 left">
									...
									<c:if test="%{borrowMFlowMarkDetail.isDayThe ==1}">个月</c:if><s:else>天</s:else>
								</td>
								<td class="f66 leftp200">
							...
							<tr>
								<td class="blue12 left">
									还款方式：
									<c:if test="%{borrowMFlowMarkDetail.paymentMode ==1}">
									   按月分期还款
									</c:if>
								...
								<td class="f66 leftp200">
								标的类型：
							<c:if test="%{borrowMFlowMarkDetail.borrowWay==1}">
							净值借款
							</c:if>
							...
							<s:else>
									...
									<td class="f66 leftp200">
	                                                                                                最高投标金额：
	                                    <c:if test="%{borrowMFlowMarkDetail.maxTenderedSum==-1}">
								                           没有限制
								        </c:if>
								        <s:else>
							...
							<tr>
								<td class="blue12 left">
									投标奖励:  
									<c:if test="%{borrowMFlowMarkDetail.excitationType ==2}">
							...
							<tr>
								<td class="blue12 left">
									投标密码：
									<c:if test="%{borrowMFlowMarkDetail.hasPWD ==1}">
									   有
									</c:if>
									<s:else>
									   无
								...
								<c:if test="%{borrowMFlowMarkDetail.borrowShow==1}">
								<tr>
								<td colspan="2" class="blue12 left">
