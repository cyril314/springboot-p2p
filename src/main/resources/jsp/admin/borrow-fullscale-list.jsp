			...
			<s:set name="acounts" value="#request.pageNum"/>
				<s:iterator value="pageBean.page" var="bean" status="s">
					<tr class="gvItem">
						...
						<td>${borrowAmount}</td>
						<td>${annualRate}%</td>
						<td>${deadline }<c:if test="%{#bean.isDayThe ==1}">个月</c:if><s:else>天</s:else></td>
						<td>
						...
						<td>满标	</td>
						<td><a href="borrowFullScaleDetail.do?id=${id}">查看</a>	</td>
					</tr>
				</s:iterator>
			</s:else>
			<tr>
				...
				<td colspan="2" >￥<c:if test="%{#request.repaymentMap.fullScaleTotalAmount==''}">0</c:if><s:else>
		        	<c:out value="#request.repaymentMap.fullScaleTotalAmount" default="0"/></s:else>
		        </td>		
			</tr>
		</tbody>
	</table>
