			...
			<div id="right">
				<div style="padding: 15px 10px 0px 10px;">
					...
					<div class="tab">
						<table cellspacing="1" cellpadding="3">
							...
							<tr>
								<td class="blue12 left">
									...
									<c:if test="%{borrowMTenderInDetail.isDayThe ==1}">个月</c:if><s:else>天</s:else>
								</td>
								<td class="f66 leftp200">
							...
							<tr>
								<td class="blue12 left">
									还款方式：
									<c:if test="%{borrowMTenderInDetail.paymentMode ==1}">
									   按月分期还款
									</c:if>
								...
								<td class="f66 leftp200">
								标的类型：
							<c:if test="%{borrowMTenderInDetail.borrowWay==1}">
							净值借款
							</c:if>
							...
							<s:else>
									...
									<td class="f66 leftp200">
	                                                                                                最高投标金额：
	                                    <c:if test="%{borrowMTenderInDetail.maxTenderedSum==-1}">
								                           没有限制
								        </c:if>
								        <s:else>
							...
							<tr>
								<td class="blue12 left">
									投标奖励:  
									<c:if test="%{borrowMTenderInDetail.excitationType ==2}">
							...
							<tr>
								<td class="blue12 left">
									投标密码：
									<c:if test="%{borrowMTenderInDetail.hasPWD ==1}">
									   有
									</c:if>
									<s:else>
									   无
							...
							<c:if test="%{borrowMTenderInDetail.borrowShow==1}">
								<tr>
								<td colspan="2" class="blue12 left">
						...
						<c:if test="%{borrowMTenderInDetail.borrowShow!=2}">
							<tr>
								<td colspan="2" align="left" style="padding-left: 30px;">
									<button id="btn_update"
										style="background-image: url('static/images/admin/btn_update.jpg'); width: 70px; border: 0; height: 26px"></button>
								   &nbsp;&nbsp;
	...
	$(function(){
	  //提交表单
   	  $('#btn_update').click(function(){
   	   	  if (falg) return ;
   		 $('#btn_update').attr('style',"background-image: url('static/images/admin/btn_chulz.jpg'); width: 70px; border: 0; height: 26px");
	     ...
	     $.shovePost('updateBorrowTenderIn.do',param,function(data){
		   ...
		   if(callBack == undefined || callBack == ''){
		       ...
		       alert(callBack);
		       falg = false;
		       $('#btn_reback').attr('style',"background-image: url('static/images/admin/btn_update.jpg'); width: 70px; border: 0; height: 26px");
