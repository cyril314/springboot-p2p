	...
	<table id="gvNews" style="width: 100%; color: #333333;" cellspacing="1" cellpadding="3" bgcolor="#dee7ef" border="0">
		<tbody>
			...
			<c:if test="pageBean.page==null || pageBean.page.size==0">
				<tr align="center" class="gvItem"><td colspan="13">暂无数据</td></tr>
			</c:if>
			<s:else>
			<s:set name="counts" value="#request.pageNum"/>
			<s:iterator value="pageBean.page" var="bean" status="s">
				<tr class="gvItem">
					<td><c:out value="#s.count+#counts"/></td>
					<td align="center">${bean.investor}</td>
					<td>
						<c:if test="#bean.realName==null">未填写</c:if>
						<s:else>${bean.realName}</s:else>
					</td>
					<td align="center">
						<c:if test="#bean.groupName==null">未分配</c:if>
						<s:else>${bean.groupName}</s:else>
					</td>
					<td>${bean.investAmount}</td>
					<td><s:number name="#bean.scale" type="#.##"/>%</td>
					...
					<td>
						<c:if test="#bean.isDayThe==1">否</c:if>
						<s:elseif test="#bean.isDayThe==2">是</s:elseif>	
					</td>
					<td>${bean.repayPeriod}</td>					
					<td>${bean.repayDate}</td>
					<td>${bean.forTotalSum}</td>
				</tr>
			</s:iterator>
			</s:else>
			<tr>
				...
				<td colspan="4" >￥<c:if test="#request.repaymentMap.amount==''">0</c:if><s:else>${repaymentMap.amount}</s:else>元</td>
		...
		<span class="require-field">待收款总合计:￥<c:if test="#request.repaymentMap.amount==''">0</c:if><s:else>${repaymentMap.amount}</s:else>元</span>
		--%>
</div>
