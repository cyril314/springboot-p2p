...
<div style="padding: 15px 10px 0px 10px;">
		...
		<table id="gvNews" style="width: 100%; color: #333333;" cellspacing="1" cellpadding="3" bgcolor="#dee7ef" border="0">
			<tbody>
				...
				<td colspan="2">￥<c:if test="#request.overduePaymentMap.totalSumm==''">0</c:if><s:else>${overduePaymentMap.totalSumm}</s:else></td>
				...
				<td colspan="2">￥<c:if test="#request.overduePaymentMap.totallateFI==''">0</c:if><s:else>${overduePaymentMap.totallateFI}</s:else></td>
				...
				<td colspan="2">￥<c:if test="#request.overduePaymentMap.amount==''">0</c:if><s:else>${overduePaymentMap.amount}</s:else></td>
		...
		<span class="require-field">逾期垫付总金额:￥<c:if test="%{#request.overduePaymentMap.amount==''}">0</c:if> <s:else><c:out value="#request.overduePaymentMap.amount" default="0"/></s:else>元</span><p/>
--></div>
