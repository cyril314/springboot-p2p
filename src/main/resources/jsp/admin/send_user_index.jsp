...
<head>
	...
	//短信本页用户发送
	function allSend() {
		if (!window.confirm("确定本页全部用户要发送短信吗?")) {
			return;
		}
		//获取id集合
		var stIdArray = [];
							...
							<tbody>
								<tr>
									<td class="f66" align="left" width="68%" height="36px">
										用户名：<input id="userName" name="paramMap.userName" type="text"/>
										姓名：<input id="realName" name="paramMap.realName" type="text"/>
										<input id="bt_search" type="button" value="查找"  />
									</td>
									<td style="">
									  <input id="bt_allIdSend" type="button" onclick="allIdSend()" value="全部发送"  />&nbsp;
									  <input id="bt_allSend" type="button" onclick="allSend()" value="本页发送"  />&nbsp;
