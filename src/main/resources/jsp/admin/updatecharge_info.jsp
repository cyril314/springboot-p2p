		     ...
		     <tbody>
		     	<tr>
								...
								<td align="left" class="f66">
								  <c:if test="#request.CostMap.type==15">
								          <c:if test="#request.maps.costMode==1">
				   ...
				   <s:iterator value="#request.Costdetallist" var="bean">
				    	...
				    	<c:if test="#bean.type==1">（100万以下奖励比率）</c:if>
				    	<c:if test="#bean.type==2">（100-200万奖励比率）</c:if>
				    	<c:if test="#bean.type==3">（200万以上奖励比率）</c:if>
				    	<br><br>
			     	</s:iterator>
				   </s:else>
								  ...
								  <td>
									...
									<input type="text" class="in_text_250" value="${maps.costFee }" id="tb_number" style="height:20px;line-height: 20px;">
										<c:if test="#request.maps.costMode==2">元</c:if>
										<s:else>%</s:else>
							...
							<tr>
								...
								<td align="left" class="f66">
									<c:if test="#request.CostMap.type==4"><span style="color: red;">注：普通用户推荐好友奖励（每推荐一个好友并加入vip得到此奖励）</span></c:if>
