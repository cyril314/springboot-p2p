...
<div class="nymain">
  ...
  <div class="sqdk" style="background:none; margin:20px">
    <div class="baozhang">
      ...
      <p>本站风险保障金累计： <c:out value="#request.totalRiskMap.total"/>元<br/>
      当日支出：<c:if test="#request.currentRiskMap.riskSpending !=''">${currentRiskMap.riskSpending}</c:if><s:else>0.00</s:else><br/>
        当日收入：<c:if test="#request.currentRiskMap.riskInCome !=''">${currentRiskMap.riskInCome}</c:if><s:else>0.00</s:else>
      </p>
    </div>
