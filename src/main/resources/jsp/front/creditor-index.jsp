...
<div class="lctab0"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <a href="userMeg.do?id=${alienatorId}" target="_blank" >${alienatorName}</a>
    </td>
    <td class="tx">
    ...
    <shove:img src="${personalHead}" defaulImg="images/hslogo_42.jpg" width="80" height="79"></shove:img>
      	...
      	<img src="{ctx}/static/images/ico_${creditratingIco}.jpg" title="${creditrating}分" />&nbsp;
      	<a href="userMeg.do?id=${publisher}" target="_blank">${borrowerName}</a></td>
    <td  align="center"><a href="queryDebtDetail.do?id=${id}" target="_blank" >${borrowTitle }</a></td>
    <td  align="center"><c:if test="#bean.isLate==2">有</c:if>
    <s:else>无</s:else></td>
    <td align="center"><c:if test="#bean.auctionMode ==1">明拍</c:if>
    	<s:else>暗拍	</s:else>
    	</td>
    ...
    <td align="center">
    	<c:if test="#bean.debtStatus ==1">申请中</c:if>
	   ...
	   <div  class="page" >
	     <shove:page url="queryFrontAllDebt.do" curPage="${pageBean.pageNum}" showPageCount="10" pageSize="${pageBean.pageSize }" theme="jsNumber" totalCount="${pageBean.totalNum}">
...
<script type="text/javascript" src="${ctx}/static/js/nav-zq.js"></script>
