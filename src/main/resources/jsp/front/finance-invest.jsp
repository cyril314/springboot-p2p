...
<div class="nymain">
<div class="ifbox1">
  <h2>确定投标 <img src="{ctx}/static/images/neiye2_06.jpg" width="4" height="7" /></h2>
        ...
        <div class="tbinfo">
          <div class="lbox">
            <div class="pic">
              ...
              <br/>
              好乐易贷信用等级：<img src="{ctx}/static/images/ico_${investDetailMap.credit}.jpg" title="${investDetailMap.creditrating}分" width="33" height="22" /></div>
              <p class="name">
...
<c:if test="%{investDetailMap.isDayThe ==1}">个月</c:if><s:else>天</s:else>
</strong></li>
<li>还款方式: <strong>
<c:if test="%{investDetailMap.paymentMode == 1}">
按月分期还款
</c:if>
...
<li>交易类型: <strong>
<c:if test="%{investDetailMap.tradeType == 1}">
线上交易
</c:if>
...
线下交易
 ...
 <div class="tbcz">
  <p class="money">
          您的帐户总额：<strong>${userMap.totalSum} 元 </strong> <a href="rechargeInit.do" class="czbtn"><img src="{ctx}/static/images/tubiao3.png" /></a><br/>
          您的可用余额：<strong>${userMap.usableSum} 元</strong> </p>
          <p class="tips">请填写并确认下面投标金额  <a href="updatexgmm.do" class="czbtn"><input type="button" value="设置交易密码"/></a><br/>
          <c:if test="#request.subscribes!=1">
	<span class="f999">最低投标金额：<c:out value="#request.minTenderedSum" default="0"/>元&nbsp;最多投标金额：<c:if test="%{investDetailMap.maxTenderedSum != -1}">${investDetailMap.maxTenderedSum}</c:if><s:else>无限制</s:else>
 	 <br/>当前年利率: ${investDetailMap.annualRate}%</span>
 	 </c:if>
 	 <s:else>
 	 	<span class="f999">最小认购金额：<c:out value="#request.investMaps.smallestFlowUnit" default="0"/>元&nbsp;
...
<input type="image" src="{ctx}/static/images/gogo_rg.gif" value="" id="invest" class="bttn_2"/>
	</div></td>
  </tr>
  <tr>  
