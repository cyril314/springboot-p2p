	...
	<div class="wrap">
		<ul class="imgb">
            ...
            <s:iterator value="#request.imgList" id="bean" status="s">
              <li class="<c:if test='#s.count == 1'>selected</c:if>"><a href="${bean.imagePath}" target="_blank">
              <shove:img src="${bean.imagePath}" title="${bean.name}" defaulImg="images/default-img.jpg"></shove:img>
              </a></li>
            </s:iterator>        
            </c:if>
			<li><img src="{ctx}/static/images/authimg.jpg" style="width:500px;height:320px;"/></li>
		</ul>
		<ul class="imgs">
			<c:if test="%{#request.imgList !=null && #request.imgList.size()>0}">
            <s:iterator value="#request.imgList" id="bean" status="s">
              <li class="<c:if test='#s.count == 1'>selected</c:if>">
              <shove:img  src="${bean.imagePath}" title="${bean.name}" defaulImg="images/default-img.jpg"></shove:img>
              </li>
            </s:iterator>        
            </c:if>
