...
<div class="nymain">
	<div class="wdzh">
		<div class="l_nav">
			<!-- 引用我的帐号主页左边栏 -->
			<div class="box"><%@include file="../include/left.jsp"%></div>
		</div>
		<div class="r_main">
			<div class="tabtil">
				<ul>
					<li class="on" onclick="jumpUrl('owerInformationInit.do');">个人详细信息	</li>
					<%-- <li onclick="loadWorkInit('queryWorkInit.do');">工作认证信息</li> --%>
					<li onclick="jumpUrl('updatexgmm.do');">修改密码	</li>
					<li id="li_bp" onclick="bindingPhoneLoad('boundcellphone.do');">更换手机	</li>
			...
			<div class="box" id="baseinfo">
				<div class="boxmain2">
					<p class="tips"><span class="fred">*</span> 为必填项，所有资料均会严格保密。<span style="color: #DB7017">*</span> &nbsp;成为借款人必填项。</p>
				</div>
				<div class="biaoge2">
					<form id="BaseDataform">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td align="right"><span style="color: red; float: none;" class="formtips">*</span>真实姓名：</td>
								<td>
									<c:if test='#request.flag=="2"'>
										<input type="text" class="inp188" name="paramMap.realName" id="realName"
									...
									<s:else>${map.realName}
										<input type="hidden" class="inp188" name="paramMap.realName" id="realName" value="${map.realName}" />
									</s:else>
								</td>
							</tr>
							<tr>
								<td align="right"><span style="color: red; float: none;" class="formtips">*</span>身份证号：</td>
								<td>
									<c:if test='#request.flag=="2"'>
										<input type="text" class="inp188" name="paramMap.idNo" id="idNo" maxlength="18"/>
							...
							<tr>
								<td align="right"><span style="color: red; float: none;" class="formtips">*</span>手机号码：</td>
								<td>
									<c:if test="#request.map.cellPhone!=null">
										<a id="cellPhone1"></a>
										<input type="hidden" class="inp188" name="paramMap.cellPhone" id="cellPhone" value="${map.cellPhone}" />
									</c:if>
									<s:else>
										<c:if test='#request.flag=="2"'>
											<input type="text" class="inp188" name="paramMap.cellPhone" id="cellPhone" />
							...
							<c:if test="#request.map.cellPhone!=null">
								<input type="hidden" class="inp100x" name="iscellPhone" id="vilidataNum" value="${map.cellPhone}" />
							</c:if>
							<s:else>
								<c:if test='#request.flag=="2"'>
								<tr>
									<td align="right"><span style="color: red; float: none;" class="formtips">*</span>手机验证码：</td>
									<td>
										<input type="text" class="inp100x" name="paramMap.vilidataNum" id="vilidataNum" />
							...
							<tr>
								...
								<td>
									...
									<%-- </c:if>
										<s:else>
											${map.sex }<input type="hidden" class="inp188" name="paramMap_sex" id="sex" value="${map.sex }" />
										</s:else>--%>
								</td>
							</tr>
							<tr>
								...
								<td>
									<%--<c:if test='#request.flag=="2"'>--%>
									<input type="text" class="inp188 Wdate" name="paramMap.birthday" id="birthday" value="${birth }"
									...
									<%-- </c:if>
									<s:else>
										${birth }<input type="hidden" class="inp188" name="paramMap.birthday" id="birthday" value="${birth }" />
									</s:else> --%></td>
							</tr>
							<tr>
								<td align="right">最高学历：</td>
								<td>
									<select name="paramMap.highestEdu" id="highestEdu" onchange="javascript:if($('#highestEdu').val()!=''){$('#u_highestEdu').html('');} " <c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>>
										<option value="">请选择</option>
										<option value="高中或以下"
											<c:if test='#request.map.highestEdu == "高中或以下"'>selected="selected"</c:if>
											<s:else></s:else>>高中或以下	</option>
										<option value="大专"
											<c:if test='#request.map.highestEdu == "大专"'>selected="selected"</c:if>
											<s:else></s:else>>大专</option>
										<option value="本科"
											<c:if test='#request.map.highestEdu == "本科"'>selected="selected"</c:if>
											<s:else></s:else>>本科</option>
										<option value="研究生或以上"
											<c:if test='#request.map.highestEdu == "研究生或以上"'>selected="selected"</c:if>
											<s:else></s:else>>研究生或以上</option>
										<option value="其他"
							...
							<tr>
								...
								<td>
									<input type="text" class="inp188 Wdate" name="paramMap.eduStartDay" id="eduStartDay"
							...
							<tr>
								...
								<td>
									<input type="text" class="inp188" name="paramMap.school" id="school" value="${map.school }"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if> />
							...
							<tr>
								...
								<td>
									<input type="radio" name="paramMap_maritalStatus" id="maritalStatus" value="已婚"
										<c:if test='#request.map.maritalStatus == "已婚"'>checked="checked"</c:if>
										<s:else></s:else> <c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										onclick="javascript:$('#u_maritalStatus').html('')"/>已婚
									<input type="radio" name="paramMap_maritalStatus" id="maritalStatus" value="未婚"
										<c:if test='#request.map.maritalStatus == "未婚"'>checked="checked"</c:if>
										<s:else></s:else> <c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										onclick="javascript:$('#u_maritalStatus').html('')" />未婚
							...
							<tr>
								...
								<td>
									<input type="radio" name="paramMap_hasChild" id="hasChild" value="有"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasChild == "有"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasChild').html('')" />有
									<input type="radio" name="paramMap_hasChild" id="hasChild" value="无"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasChild == "无"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasChild').html('')" />无
									<span style="color: red; float: none;" id="u_hasChild" class="formtips"></span>
								</td>
							</tr>
							<tr>
								<td align="right">是否有房：</td>
								<td>
									<input type="radio" name="paramMap_hasHourse" id="hasHourse" value="有"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasHourse == "有"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasHourse').html('')" />有
									<input type="radio" name="paramMap_hasHourse" id="hasHourse" value="无"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasHourse == "无"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasHourse').html('')" />无
									<span style="color: red; float: none;" id="u_hasHourse"	class="formtips"></span>
								</td>
							</tr>
							<tr>
								<td align="right">有无房贷：</td>
								<td>
									<input type="radio" name="paramMap_hasHousrseLoan" id="hasHousrseLoan" value="有"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasHousrseLoan == "有"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasHousrseLoan').html('')" />有
									<input type="radio" name="paramMap_hasHousrseLoan" id="hasHousrseLoan" value="无"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasHousrseLoan == "无"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasHousrseLoan').html('')" />无
							...
							<tr>
								...
								<td>
									<input type="radio" name="paramMap_hasCar" id="hasCar" value="有"
										...
										<s:else></s:else> onclick="javascript:$('#u_hasCar').html('')" />有
									<input type="radio" name="paramMap_hasCar" id="hasCar" value="无"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasCar == "无"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasCar').html('')" />无
									<span style="color: red; float: none;" id="u_hasCar" class="formtips"></span>
								</td>
							</tr>
							<tr>
								<td align="right">有无车贷：</td>
								<td>
									<input type="radio" name="paramMap_hasCarLoan" id="hasCarLoan" value="有"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasCarLoan == "有"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasCarLoan').html('')" />有
									<input type="radio" name="paramMap_hasCarLoan" id="hasCarLoan" value="无"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if>
										<c:if test='#request.map.hasCarLoan == "无"'>checked="checked"</c:if>
										<s:else></s:else> onclick="javascript:$('#u_hasCarLoan').html('')" />无
							...
							<tr id="jg">
								...
								<td>
									<s:select id="province" name="workPro"
										...
										list="#request.provinceList" listKey="regionId"	listValue="regionName" headerKey="-1" headerValue="--请选择--"
										onchange="javascript:if($('#province').val()!=-1){$('#u_Pro_City').html('')}" />
									<s:select id="city" name="cityId"
										cssStyle="border-left:1px solid #dedede;border-top:1px solid #dedede;border-right:1px solid #dedede;border-bottom:1px solid #dedede;"
										list="#request.cityList" listKey="regionId"	listValue="regionName" headerKey="-1" headerValue="--请选择--"
										onchange="javascript:if($('#city').val()!=-1){$('#u_Pro_City').html('')}" />
									<span style="color: red; float: none;" id="u_Pro_City" class="formtips"></span>
								</td>
							</tr>
							<tr id="hksz">
								...
								<td>
									...
									<s:select id="registedPlaceCity" name="regCity"	cssStyle="border-left:1px solid #dedede;border-top:1px solid #dedede;border-right:1px solid #dedede;border-bottom:1px solid #dedede;"
							...
							<tr id="jzdz">
								...
								<td>
									<input type="text" class="inp188" name="paramMap.address" id="address" value="${map.address}"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if> />
									格式：**省***市**区***大街146号&nbsp;
							...
							<tr id="jzdh">
								...
								<td>
									<input type="text" class="inp188" name="paramMap.telephone"	id="telephone" value="${map.telephone}"
										<c:if test='#request.map.auditStatus ==3'>disabled="disabled"</c:if> />
									格式：0311-29556666 或 010-83881140&nbsp;
									<span style="color: red; float: none;" id="u_telephone"	class="formtips"></span>
								</td>
							</tr>
							<tr id="grtx">
								<td align="right" id="personalHead">个人头像：</td>
								<td>
							...
							<tr>
								...
								<td class="tishi">
									...
									一旦被发现所填资料有虚假，将会影响您在好乐易贷的信用，对以后借款造成影响。
								</td>
							</tr>
							<tr>
								<td align="right">&nbsp;</td>
								<td style="padding-top: 20px;">
									<!-- <c:if test="#session.user.map.auditStatus==3"></c:if>
										<s:else> -->
