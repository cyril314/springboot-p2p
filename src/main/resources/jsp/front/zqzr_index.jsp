...
<div class="nymain">
	<div class="lcnav">
		<div class="tab">
			<div class="tabmain">
				<ul><li class="on" style="padding:0 20px;">债权转让及受让协议</li></ul>
			</div>
		</div>
		<div class="line"></div>
	</div>
		...
		<div class="lcmain_l" style="width: 98%">
			<div class="lctab" style="padding:0 10px; width: 98%">
				<div class="gginfo">
					...
					<p>出借编号：${map.borrowId}${map.id }<br/>
						尊敬的 
						<u>
							&nbsp;&nbsp;<c:if test="#request.map.dbrealNam==''">${map.username }</c:if>
							<s:else>${map.dbrealName }</s:else>&nbsp;&nbsp;
						</u>  
						先生/女士，您好！通过好乐易贷资质评估与筛选，推荐您以受让他人既有的借贷合同的方式，通过p2p网络平台的方式，自助出借资金给如下借款人，详见《债权列表》，在您接受该批债权转让时，预期您的出借获益情况如下：
	    			</p>
					<h2>债权列表</h2>
					<ul>
						<li style="width: 200px;" >
							转让人（原债权人）
							<c:if test="#request.map.realName==''">${map.username }</c:if>
							<s:else>${map.realName }</s:else>
						</li>
						<li style="width: 350px;">身份证号码：  ${map.idNo}</li>
						<li style="width: 200px;">平台用户名 ： ${map.username }</li>
					</ul>
					<ul>
						<li style="width: 200px;" >
							受让人（新债权人）：
							<c:if test="#request.map.dbrealName==''">${map.dbusername }</c:if>
							<s:else>${map.dbrealName }</s:else>
						</li>
						<li style="width: 350px;">身份证号码 ： ${map.dbisno }</li>
						<li style="width: 200px;">平台用户名 ： ${map.dbusername }</li>
					</ul>
