...
<div class="ny_main1 clearfix">
	<div class="myhome_main clearfix">
        ...
        <div class="myhome_r clearfix">
        	 ...
        	 <div class="hh_zh">
             	 <span class="zh_vip">
	             	 <c:if test="#session.user.vipStatus!=2"><a href="quervipData.do"></a></c:if>
	             	 <s:else></s:else>
             	 </span>
             	  <ul class="zh_ul">
                  	  <li><em title="账户总额 = 可用金额 + 冻结金额 + 待收金额 + 可用理财专用金 + 冻结理财专用金">账户总额 </em>: 
                  	  	<span>￥
                  	  		<c:out value="#request.accmountStatisMap.accountSum" default="0.00"/>
							<!--<c:if test="#request.accmountStatisMap.accountSum !=''">${accmountStatisMap.accountSum+accmountStatisMap.usableGold+accmountStatisMap.freezeGold}</c:if>
							<s:else>0.00</s:else>-->
						</span>
						</li>
                      <li>冻结金额 : <span>￥
							<c:if test="#request.accmountStatisMap.freezeAmount !=''">${accmountStatisMap.freezeAmount}</c:if>
							<s:else>0.00</s:else></span></li>
                      <li>可用余额 : <span>￥
							<c:if test="#request.accmountStatisMap.usableAmount !=''">${accmountStatisMap.usableAmount}</c:if>
							<s:else>0.00</s:else></span>
						</li>
                  </ul>
                  <div class="zh_btn">
                  	  <a href="rechargeInit.do" class="zh_cz">充 值</a>
                      <a href="withdrawLoad.do" class="zh_tx">提 现</a>
                  </div>
                  <div class="clear"></div>
             </div>
             <!--账户概况 结束-->
